// -*- C++ -*-
//
// Class:      EcalTPGAnalyzerLaserCorr
//
//
...
#include "EcalTPGAnalyzerLaserCorr.h"
...
EcalTPGAnalyzerLaserCorr::EcalTPGAnalyzerLaserCorr(const edm::ParameterSet&  iConfig)
{
...
EcalTPGAnalyzerLaserCorr::~EcalTPGAnalyzerLaserCorr()
{
  file_->cd();
  tree_->Write();
  //treeShape_->Write();
...
void EcalTPGAnalyzerLaserCorr::beginRun( const edm::EventSetup & iSetup,const edm::Run & iRun)
{
  myevt = 0;
...
void EcalTPGAnalyzerLaserCorr::analyze(const edm::Event& iEvent, const  edm::EventSetup & iSetup)
{
...
cout<<"obtained active trigger info"<<endl;
 ...
 edm::Handle<EcalTrigPrimDigiCollection> tp;
  ...
  if (!noL1Info) {
    ...
    // 	if (itp->hwPt() <=0) continue; 
    // 	if (itp->hwIso()==0){
    // 	  treeVariables_.L1preNonisoIeta[isocounter1] = itp->hwEta();
    // 	  treeVariables_.L1preNonisoIphi[isocounter1] = itp->hwPhi();
    // 	  treeVariables_.L1preNonisoRank[isocounter1] = itp->hwPt(); 
    // 	  nonisocounter1++;
    // 	}
    ...
    //   if ( itp->hwPt() <=0 ) continue; 
    //   if (itp->hwIso()==0){ 
    // 	treeVariables_.L1NonisoIeta[isocounter1] = itp->hwEta();
    // 	treeVariables_.L1NonisoIphi[isocounter1] = itp->hwPhi();
    // 	treeVariables_.L1NonisoRank[isocounter1] = itp->hwPt(); //Do we have the rank now?
    // 	nonisocounter1++;
    //   }
    //   //cout << __LINE__ << endl;
    //   if (itp->hwIso()==1) {
  ...
  //   	if ( itp->hwPt() <=0 ) continue; 
  //   	if (itp->hwIso()==0){
  // 	  treeVariables_.L1postNonisoIeta[isocounter1] = itp->hwEta();
  // 	  treeVariables_.L1postNonisoIphi[isocounter1] = itp->hwPhi();
  // 	  treeVariables_.L1postNonisoRank[isocounter1] = itp->hwPt(); //Do we have the rank now?
  // 	  nonisocounter1++;
  // 	}
  //   	if (itp->hwIso()==1) {
...
DEFINE_FWK_MODULE(EcalTPGAnalyzerLaserCorr);
