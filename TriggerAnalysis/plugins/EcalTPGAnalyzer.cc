...
EcalTPGAnalyzer::EcalTPGAnalyzer(const edm::ParameterSet&  iConfig)
{
  ...
  // ADDED DP
  tree_->Branch("nTT_1ADC", &treeVariables_.nTT_1ADC,"nTT_1ADC/I");
  tree_->Branch("nTT_2ADC", &treeVariables_.nTT_2ADC,"nTT_2ADC/I");
  tree_->Branch("nTT_3ADC", &treeVariables_.nTT_3ADC,"nTT_3ADC/I");
  tree_->Branch("nTT_4ADC", &treeVariables_.nTT_4ADC,"nTT_4ADC/I");
  // ADDED DP
 
  // ADDED DP
  tree_->Branch("nTTemu_1ADC", &treeVariables_.nTTemu_1ADC,"nTTemu_1ADC/I");
  tree_->Branch("nTTemu_2ADC", &treeVariables_.nTTemu_2ADC,"nTTemu_2ADC/I");
  tree_->Branch("nTTemu_3ADC", &treeVariables_.nTTemu_3ADC,"nTTemu_3ADC/I");
  tree_->Branch("nTTemu_4ADC", &treeVariables_.nTTemu_4ADC,"nTTemu_4ADC/I");
  // ADDED DP
...
///if(isRECO){  ///commented by me bcoz not geeting nvtx plot for data
...
cout<<"obtained active trigger info"<<endl;
 ...
 edm::Handle<EcalTrigPrimDigiCollection> tp;
	...
	mapTower[TPtowid] = tE ;
        ...
        // ADDED DP
        if (abs(TPtowid.ieta())<=4) {
        if ((d[0].raw()&0xff)>=1) count_1ADC++;
        if ((d[0].raw()&0xff)>=2) count_2ADC++;
        if ((d[0].raw()&0xff)>=3) count_3ADC++;
        if ((d[0].raw()&0xff)>=4) count_4ADC++;
       }
        // ADDED DP
    ...
    // ADDED DP
    treeVariables_.nTT_1ADC=count_1ADC;
    treeVariables_.nTT_2ADC=count_2ADC;
    treeVariables_.nTT_3ADC=count_3ADC;
    treeVariables_.nTT_4ADC=count_4ADC;
    // ADDED DP
  ...
  cout<<"obtained L1 event infor"<<endl;
   ...
   // ADDED DP
    ...
    for (unsigned int i=0;i<tpEmul.product()->size();i++) {
     ...
     // ADDED DP
     if (abs(TPtowid.ieta())<=4) {
     if ((d[2].raw()&0xff)>=1) countemu_1ADC++;
     if ((d[2].raw()&0xff)>=2) countemu_2ADC++;
     if ((d[2].raw()&0xff)>=3) countemu_3ADC++;
     if ((d[2].raw()&0xff)>=4) countemu_4ADC++;
    }
    // ADDED DP
    ...
    // ADDED DP
    treeVariables_.nTTemu_1ADC=countemu_1ADC;
    treeVariables_.nTTemu_2ADC=countemu_2ADC;
    treeVariables_.nTTemu_3ADC=countemu_3ADC;
    treeVariables_.nTTemu_4ADC=countemu_4ADC;
    // ADDED DP
...
///*if(towid.ieta() > 600 || towid.ieta() < -600)*/cout << "Tower Characters " << towid.ieta() << "\t" << towid.iphi() << endl;
	...
	if (itTT != mapTower.end()) {
	  ...
	  double theta = theBarrelGeometry_->getGeometry(id)->getPosition().theta() ;
          ////std::cout << " theBarrelGeometry_ " << theBarrelGeometry_ << std::endl;
