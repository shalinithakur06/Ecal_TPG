...
{  
  ...
  if (DEBUG) cout << __LINE__ << " " << algos1.size() << endl ;
  ...
  TH1F * occupancyTPEmulEEPlus_Eta = new TH1F("occupancyTPEmulEEPlus_Eta", "Emulated TP Occupancy vs #eta: Plus Endcap", 11, 18, 29) ;
  occupancyTPEmulEEPlus_Eta->GetYaxis()->SetTitle("eta index") ;
  TH1F * occupancyTPEmulEEMinus_Eta = new TH1F("occupancyTPEmulEEMinus_Eta", "Emulated TP Occupancy vs #eta: Plus Minus", 11, -28, -17) ;
  occupancyTPEmulEEMinus_Eta->GetYaxis()->SetTitle("eta index") ;
 ...
 //  TH2F * MaskedReadOutEEMinus = new TH2F("MaskedReadOutEEMinus", "MaskedReadOutEEMinus", 121, -10, 111, 121, -10, 111) ;
	  ...
	  if (geometry.size()>0)
		...
		{  
		  ...
		  if (tp>occupancyCut && emul[ref]>0){
		    occupancyTPEmulEEPlus_Eta->Fill(ieta);
		  }
		  for (uint i=0; i !=geometry[hashedIndex].size();++i)
		    {
		      if (tp>occupancyCut && emul[ref]>0) 
	      ...
	      if (ieta<=-18 )
		{
		  ...
		  if (tp>occupancyCut && emul[ref]>0){
		    occupancyTPEmulEEMinus_Eta->Fill(ieta);
		  }
		  for (uint i=0; i !=geometry[hashedIndex].size();++i)
		    {
		      if (tp>occupancyCut && emul[ref]>0)
  ...
  occupancyTPEmulEEPlus_Eta->Write() ;
  occupancyTPEmulEEMinus_Eta->Write() ;
