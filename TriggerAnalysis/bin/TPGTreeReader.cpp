...
std::vector<std::string> split(std::string str, std::string separator )
{
    ...
    do{
      ...
      while(*cstr != c && *cstr)
	  cstr++;
      if (string(begin, cstr).size())  result.push_back(string(begin, cstr));
    } while (0 != *cstr++);
...
{  
  ...
  std::string geomName = "/afs/cern.ch/work/n/ndev/TPG/CMSSW_8_0_5/src/CMS-ECAL_TPGAnalysis/Scripts/TriggerAnalysis/macros/endcapGeometry.txt";
  if (DEBUG) cout << __LINE__ << endl ;
  ...
  if (DEBUG) cout << __LINE__ << endl ;
  std::vector<std::string> algos1 = split(l1algo,",") ;
  cout << __LINE__ << " " << algos1.size() << endl ;
  for (uint i=0 ; i<algos1.size() ; i++) limits.push_back(atoi(algos1[i].c_str()));
  if (DEBUG) cout << __LINE__ << " " << limits.size() << endl ;
  // if (limits.size()!=0) {
  //   for (int i=limits[0]; i<limits[1]+1; i++ ) algobits.push_back(i);
  //   cout << __LINE__ << " " << limits[0] << " " << limits[1] <<  algobits.size() << endl;
  // }
  for (uint i=0 ; i<limits.size() ; i++) algobits.push_back(limits[i]) ;

  if (DEBUG) cout << __LINE__ << endl ;

  std::vector<int> bxlimits; 
  std::vector<int> algobitsbx ;
  //  std::vector<std::string> algosbx = split(l1algobx,",") ;
  std::vector<std::string> algosbx1 = split(l1algobx,",") ;
  for (uint i=0 ; i<algosbx1.size() ; i++) bxlimits.push_back(atoi(algosbx1[i].c_str()));
  if (bxlimits.size()!=0){
    for (int i=bxlimits[0]; i<bxlimits[1]+1; i++ ) algobitsbx.push_back(i);
  }
  ...
  cout <<__LINE__ << " "<< techTrgStringVector.size() << endl;
  for (uint i=0 ; i<techTrgStringVector.size() ; i++)
    {
      bool exclude = false;
  ...
  std::vector<std::string> lumiStringVector;
  
  if(lumi != "-1"){
    lumiStringVector = split(lumi,",") ;
  ...
  std::vector<std::string> eventStringVector ;
  if(levent != "-1"){
    eventStringVector = split(levent,",") ;
  ...
  if (timeBin > 100000) timeBin = 100000;
  std::cout << "time bin: " << timeBin << " " << slMaxTime << " " << slMinTime << std::endl;
  ...
  cout << __LINE__ << " " << timeBin << " " << slMaxTime-slMinTime << endl;
  ...
  TH1F * nbOfL1IsoCands = new TH1F("nbOfL1IsoCands", "nbOfL1IsoCands", 9, 0., 9.) ;
  TH1F * nbOfL1NonisoCands = new TH1F("nbOfL1NonisoCands", "nbOfL1NonisoCands", 9, 0., 9.) ; 
  ...
  TH1F * nbOfL1preIsoCands = new TH1F("nbOfL1preIsoCands", "nbOfL1preIsoCands", 9, 0., 9.) ;
  TH1F * nbOfL1preNonisoCands = new TH1F("nbOfL1preNonisoCands", "nbOfL1preNonisoCands", 9, 0., 9.) ; 
  ...
  TH1F * nbOfL1postIsoCands = new TH1F("nbOfL1postIsoCands", "nbOfL1postIsoCands", 9, 0., 9.) ;
  TH1F * nbOfL1postNonisoCands = new TH1F("nbOfL1postNonisoCands", "nbOfL1postNonisoCands", 9, 0., 9.) ; 
 ...
 //barrel L1 TP comparisons
  ...
  for (int entry = firstEntry ; entry < lastEntry ; ++entry)
	...
	for (uint ntrig = 0 ; ntrig < treeVars.nbOfActiveTriggers ; ntrig++)
      ...
      if (keep) continue ;*/
      keep=false ;
      if (algobits.size()) cout << __LINE__ <<  " " << entry<< " "  << algobits.size() << endl;
	...
	for (uint iMaskRCT = 0 ; iMaskRCT < treeVars.nMaskedRCT ; iMaskRCT++) {
	  ...
	  //cout << __LINE__ << " " << rcteta << " " << rctphi << endl; 
      ...
      for (uint l1isocand = 0 ; l1isocand < treeVars.nbOfL1IsoCands ; l1isocand++)
	{
	  ...
	  if(treeVars.L1IsoRank[l1isocand]>=eg)
	    { //require EG2 by default, EG eg in general
	      ...
	      std::vector<int> tteta ;
	      tteta.push_back(treeVars.L1IsoIeta[l1isocand]);
	      std::vector<int> ttphi ;
	      ttphi.push_back( treeVars.L1IsoIphi[l1isocand]);
      ...
      for (uint l1nonisocand = 0 ; l1nonisocand < treeVars.nbOfL1NonisoCands ; l1nonisocand++)
	{
	  if(treeVars.L1NonisoRank[l1nonisocand]>=eg)
	    { //require EG2 by default, EG eg in general
	      ...
	      std::vector<int> tteta ;
	      tteta.push_back(treeVars.L1NonisoIeta[l1nonisocand]);
	      std::vector<int> ttphi ;
	      ttphi.push_back(treeVars.L1NonisoIphi[l1nonisocand]);
	      if(abs(tteta[0])<17){//EB
      ...
      for (uint l1preisocand = 0 ; l1preisocand < treeVars.nbOfL1preIsoCands ; l1preisocand++)
	{
	  if(treeVars.L1preIsoRank[l1preisocand]>=0)
		...
		{ //require EG2 by default, EG eg in general
		  ...
		  std::vector<int> tteta ;
		  tteta.push_back(treeVars.L1preIsoIeta[l1preisocand]);
		  std::vector<int> ttphi ;
		  ttphi.push_back(treeVars.L1preIsoIphi[l1preisocand]);
      ...
      for (uint l1prenonisocand = 0 ; l1prenonisocand < treeVars.nbOfL1preNonisoCands ; l1prenonisocand++)
	{
	  if(treeVars.L1preNonisoRank[l1prenonisocand]>=2)
		...
		{ //require EG2 by default, EG eg in general
		  ...
		  std::vector<int> tteta ;
		  tteta.push_back(treeVars.L1preNonisoIeta[l1prenonisocand]);
		  std::vector<int> ttphi ;
		  ttphi.push_back(treeVars.L1preNonisoIphi[l1prenonisocand]);
      ...
      for (uint l1postisocand = 0 ; l1postisocand < treeVars.nbOfL1postIsoCands ; l1postisocand++)
	{
	  if(treeVars.L1postIsoRank[l1postisocand]>=2)
	      ...
	      if(treeVars.L1postIsoRank[l1postisocand]>=eg)
		{
		  ...
		  std::vector<int> tteta;
		  tteta.push_back(treeVars.L1postIsoIeta[l1postisocand]);
		  std::vector<int> ttphi ;
		  ttphi.push_back(treeVars.L1postIsoIphi[l1postisocand]);
      ...
      for (uint l1postnonisocand = 0 ; l1postnonisocand < treeVars.nbOfL1postNonisoCands ; l1postnonisocand++)
	{
	  if(treeVars.L1postNonisoRank[l1postnonisocand]>=2)
	      ...
	      if(treeVars.L1postNonisoRank[l1postnonisocand]>=eg)
		{
		  ...
		  std::vector<int> tteta ;
		  tteta.push_back(treeVars.L1postNonisoIeta[l1postnonisocand]);
		  std::vector<int> ttphi ;
		  ttphi.push_back(treeVars.L1postNonisoIphi[l1postnonisocand]);
		  if(abs(tteta[0])<17){//EB
		    L1NonisotimingEB->Fill(1);
		  }
		  if(tteta[0]>=17){//EE+
      ...
      int MaxTPinpreIsoRegion[8];
      int MaxTPinpreIsoRegionIeta[8];
      int MaxTPinpreIsoRegionIphi[8];
      int SecondTPinpreIsoRegion[8];
     

       
      int MaxTPinpreIsoRegion_min2[8];
      int MaxTPinpreIsoRegion_min2Ieta[8];
      int MaxTPinpreIsoRegion_min2Iphi[8];
      int SecondTPinpreIsoRegion_min2[8];
     
 
      int MaxTPinpreIsoRegion_min1[8];
      int MaxTPinpreIsoRegion_min1Ieta[8];
      int MaxTPinpreIsoRegion_min1Iphi[8];
      int SecondTPinpreIsoRegion_min1[8];
     
 
      int MaxTPinpreIsoRegion_corrbx[8];
      int MaxTPinpreIsoRegion_corrbxIeta[8];
      int MaxTPinpreIsoRegion_corrbxIphi[8];
      int SecondTPinpreIsoRegion_corrbx[8];
     
 
      int MaxTPinpreIsoRegion_plus1[8];
      int MaxTPinpreIsoRegion_plus1Ieta[8];
      int MaxTPinpreIsoRegion_plus1Iphi[8];
      int SecondTPinpreIsoRegion_plus1[8];
     
 
      int MaxTPinpreIsoRegion_plus2[8];
      int MaxTPinpreIsoRegion_plus2Ieta[8];
      int MaxTPinpreIsoRegion_plus2Iphi[8];
      int SecondTPinpreIsoRegion_plus2[8];
 ...
 //davids special treatment
 //if (tp<33 || indexOfTPEmulMax!=2) continue; 
 if(tp >=33 && indexOfTPEmulMax ==2){

	realminusemulTPvsieta->Fill(ieta,tp-maxOfTPEmul);
	realminusemulTPratvsieta->Fill(ieta,float(tp-maxOfTPEmul)/float(tp));

	// barrel
	
	if (abs(ieta)<18) {

	  // write matched TPs to file
	  
	  //ofs << runNb << " " << evtNb <<  " " << ieta << " "
	  //    << iphi << endl;

	  count++;

	  realTP->Fill(tp);
	  emulTP->Fill(maxOfTPEmul);

	  if (severity_level>2) {
	    countsev++;
	    realTP_sev->Fill(tp);
	    emulTP_sev->Fill(maxOfTPEmul);
	  }

	  if (severity_level==3) countsev3++;
	  if (severity_level==4) countsev4++;
	  
	  
	  realminusemulTP->Fill(tp-maxOfTPEmul);
	  realminusemulTPvsreal->Fill(tp,tp-maxOfTPEmul);
	  realvsemulTP->Fill(tp,maxOfTPEmul);

	

	  // TPs with matching Et
	  
	  if (tp==maxOfTPEmul) {

	    if (debug) {
	      cout << " MATCHED: ieta=" << ieta << " iphi=" << iphi
		   << "\n    REAL TP: Et=" << tp << " TTF=" << ttflag
		   << " sFGVB=" << raw_spike 
		   << "\n    EMUL TP: Et=" << maxOfTPEmul << " TTF=" << emulttf
		   << " sFGVB=" << emulsfgvb 
		   << " index=" << indexOfTPEmulMax
		   << "      Nxtal=" << nxtals << " sevlv=" << severity_level 
		   << endl;
	    }

	    etaphimatch->Fill(ieta,iphi);
	    
	  }
	  
	  // tps with mismatched Et
	  
	  else {
	  
	    if (debug) {
	      cout << " MISMATCHED: ieta=" << ieta << " iphi=" << iphi 
		   << "\n    REAL TP: Et=" << tp << " TTF=" << ttflag
		   << " sFGVB=" << raw_spike 
		   << "\n    EMUL TP: Et=" << maxOfTPEmul << " TTF=" << emulttf
		   << " sFGVB=" << emulsfgvb 
		...
		cout << "    BX=" << i-2 << " Et=" << emul[i]
		     << " TTF=" << emulTTF[i] << " sFGVB=" << emulsFGVB[i] << endl; 
	      }

	    }
	      
	    etaphimismatch->Fill(ieta,iphi);
	...
	// endcaps
	  ...
	  realTPEE->Fill(tp);
	  emulTPEE->Fill(maxOfTPEmul);
	
	  realminusemulTPEE->Fill(tp-maxOfTPEmul);
	  realminusemulTPvsrealEE->Fill(tp,tp-maxOfTPEmul);
	  realvsemulTPEE->Fill(tp,maxOfTPEmul);

	
	
	  if (tp==maxOfTPEmul) {

	    etaphimatchEE->Fill(ieta,iphi);
	    
	  }
	  
	  else {
	    
	    etaphimismatchEE->Fill(ieta,iphi);
	  ...
	  if(tp>occupancyCut) {
	    ...
	    if (DEBUG) cout << __LINE__ << endl ;
	    if (DEBUG) cout << __LINE__ << "  " << treeVars.timeStamp/60. << " " <<slMinTime << endl ; 
	    if (DEBUG) cout << __LINE__ << "  " << iphi << " " << ieta << endl;
	    time3D->Fill(iphi,ieta,0);//treeVars.timeStamp/60. - slMinTime);
	    if (DEBUG) cout << __LINE__ << endl ;
  ...
  emulTP->Write();
  emulTP_sev->Write();
  realminusemulTP->Write();
  realminusemulTPvsreal->Write();
  realvsemulTP->Write();
  etaphimatch->Write();
  etaphimismatch->Write();


  realTPEE->Write();
  emulTPEE->Write();
  realminusemulTPEE->Write();
  realminusemulTPvsrealEE->Write();
  realvsemulTPEE->Write();
  etaphimatchEE->Write();
  etaphimismatchEE->Write();

  realminusemulTPvsieta->Write();
  realminusemulTPratvsieta->Write();
  TPEtaEB->Write() ;
  TPPhiEB->Write() ;
  TPEtaEEPlus->Write() ;
  TPPhiEEPlus->Write() ;
  ...
  if (DEBUGEND) cout << __LINE__ << endl ;
  ...
  algosbx1.~vector(); 
  if (DEBUGEND) cout << __LINE__ << endl ;
  bcStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  bcRangeStringVector.~vector();
  bcRangeIntVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  bcIncludeVector.~vector() ;
  bcExcludeVector.~vector() ;
  if (DEBUGEND) cout << __LINE__ << endl ;
  bcStringVector.~vector() ; 
  vmytechtriggers.~vector();    
  if (DEBUGEND) cout << __LINE__ << endl ;
  techTrgRangeStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  techTrgStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  eventRangeStringVector.~vector();
   
  orbitRangeStringVector.~vector();
  orbitStringVector.~vector();
   
  timestampRangeStringVector.~vector();
  timestampStringVector.~vector();
   
  

  if (DEBUGEND) cout << __LINE__ << endl ;
  lumiRangeStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  lumiStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  lumiRangeIntVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  lumiIncludeVector.~vector() ;
  if (DEBUGEND) cout << __LINE__ << endl ;
  lumiExcludeVector.~vector() ;
  if (DEBUGEND) cout << __LINE__ << endl ;
  towerRangeStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  towerIncludeVector.~vector() ;
  if (DEBUGEND) cout << __LINE__ << endl ;
  towerExcludeVector.~vector() ;
  //if (DEBUGEND) cout << __LINE__ << endl ;
  //lumiStringVector.~vector();
  if (DEBUGEND) cout << __LINE__ << endl ;
  towerStringVector.~vector();
  eventStringVector.~vector();
