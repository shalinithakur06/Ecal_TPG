#!/bin/bash
#Q1: what is the impact in the code of specifying the trigger value. What is the default value of the trigger. 
# givining the impression that is L1 Trigger threshold.

usage='Usage: -r <run number> ( -d <eos dir> -l <html dir> -p <http dir> -e <EG trigger value for L1> )'

echo " "
#this is the command to read the command line arguments in a systematic way.
args=`getopt rdlp: -- "$@"`
if test $? != 0
     then
         echo $usage
...
#understand and explain all the command line options and also check if they are implemented correctly in the code. 
eval set -- "$args"
...
TPGPLOTTINGDIR=${CMSSW_BASE}/src/EcalTPGAnalysis/TPGPlotting
...
if [ "X"${EOSDIR} == "X" ]
   then
   EOSDIR=/eos/user/${myusername:0:1}/${myusername}/TPG/
   echo "using default EOSDIR:" $EOSDIR
fi
  
if [ "X"${HTMLDIR} == "X" ]
    then
    HTMLDIR="/afs/cern.ch/user/${myusername:0:1}/${myusername}/www/TPGAnalysis/Run2018/"
    echo "using default HTMLDIR:" $HTMLDIR
fi

if [ "X"${HTTPDIR} == "X" ]
    then
    HTTPDIR="http://${myusername}.web.cern.ch/${myusername}/TPGAnalysis/Run2017/"
...
#PFNDIR="root://eoscms.cern.ch//eos/cms/store/caf/user/ccecal/TPG"
#PFNDIR="root://eoscms.cern.ch//eos/cms/store/caf/user/taroni/TPG"
#PFNDIR="root://eoscms.cern.ch//eos/cms/store/caf/user/taroni/TPG/"
#PFNDIR="/tmp/${myusername}/"
PFNDIR="${CMSSW_BASE}/src/EcalTPGAnalysis/Scripts/TriggerAnalysis/"
echo PFNDIR=$PFNDIR
...
/*  TFile *_file0 = TFile::Open("${PFNDIR}/ECALTPGtree_${run}.root");
...
echo ""
echo $ADDENDUM
echo " $MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReader -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL  -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_$run$APPENDIX.root -l1bx 39,62  -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS"

$MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReader -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL  -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_$run$APPENDIX.root  -l1bx 39,40,41,42,43,44,45,46,48,50,52  -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS
cp -f newhistoTPG_$run$APPENDIX.root ${EOSDIR}/newhistoTPG_$run$APPENDIX.root
mv newhistoTPG_$run$APPENDIX.root $TPGPLOTTINGDIR/plots/.
echo ""
echo "TPGTreeReader running on Ecal triggers..."
$MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReader -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_Ecal_$run$APPENDIX.root -l1 39,40,41,42,43,44,45,46,48,50,52   -l1bx 39,40,41,42,43,44,45,46,48,50,52 -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS
cp -f newhistoTPG_Ecal_$run$APPENDIX.root ${EOSDIR}/newhistoTPG_Ecal_$run$APPENDIX.root
