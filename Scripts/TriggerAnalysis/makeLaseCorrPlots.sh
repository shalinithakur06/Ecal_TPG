...
if [ "X"${EOSDIR} == "X" ]
   then
   EOSDIR=/store/caf/user/taroni/TPG/
...
if [ "X"${HTMLDIR} == "X" ]
   ...
   # HTMLDIR=/afs/cern.ch/user/c/ccecal/scratch0/www/TPGAnalysis/Commissioning2015
    HTMLDIR='/afs/cern.ch/user/t/taroni/www/TPGAnalysis/Run2016B'
    echo "using default HTMLDIR:" $HTMLDIR
fi

if [ "X"${HTTPDIR} == "X" ]
    then
    HTTPDIR='http://taroni.web.cern.ch/taroni/TPGAnalysis/Run2016B'
...
#PFNDIR="root://eoscms.cern.ch//eos/cms/${EOSDIR}"
PFNDIR="/tmp/taroni/"
echo PFNDIR=$PFNDIR
...
echo ""
echo $ADDENDUM
echo " $MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReader -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL   -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_$run$APPENDIX.root -l1bx 47,49,50,51,52,53  -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS
"
$MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReaderLaser -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL  -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_$run$APPENDIX.root -l1bx 47,49,50,51,52,53  -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS
cmsStage -f newhistoTPG_$run$APPENDIX.root ${EOSDIR}/newhistoTPGLaser_$run$APPENDIX.root
mv newhistoTPG_$run$APPENDIX.root $TPGPLOTTINGDIR/plots/.
echo ""
echo "TPGTreeReader running on Ecal triggers..."
#$MYRELEASE/bin/${SCRAM_ARCH}/TPGTreeReaderLaser -SLMinEvt $MINevtVAL -SLMaxEvt $MAXevtVAL -SLMinOrb $MINorbVAL -SLMaxOrb $MAXorbVAL -SLMaxTime $MAXtimeVAL -SLMinTime $MINtimeVAL -d $PFNDIR -i ECALTPGtree${ADDENDUM}_${run}.root -o newhistoTPG_Ecal_$run$APPENDIX.root -l1 39,62  -l1bx 39,62 -g $TPGPLOTTINGDIR/plots/endcapGeometry.txt -EG $EG $OPTIONS
#cmsStage -f newhistoTPG_Ecal_$run$APPENDIX.root ${EOSDIR}/newhistoTPGLaser_Ecal_$run$APPENDIX.root
