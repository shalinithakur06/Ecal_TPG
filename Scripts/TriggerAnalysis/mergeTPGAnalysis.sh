...
hadd -f -k /tmp/${USER}/ECALTPGtree${addendum}_${run_num}.root ${merge_dir}/ECALTPGtree_*root

if [ "X"${output_dir} == "X" ]
    then
   # 	output_dir=eos/cms/store/caf/user/ccecal/TPG/
    	output_dir=eos/cms/store/caf/user/${USER}/TPG/${run_num}/
    	...
    	xrdcp -f /tmp/${USER}/ECALTPGtree${addendum}_${run_num}.root ${output_root_dir} 
   	 ...
   	 eosmkdir ${output_dir}
   	 eos chmod 775 ${output_dir}
    	 xrdcp /tmp/${USER}/ECALTPGtree${addendum}_${run_num}.root ${output_dir} 
    	#rfchmod 775 ${output_dir}/ECALTPGtree_${addendum}_${run_num}.root
fi
